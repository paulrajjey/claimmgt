<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("listdocout");
					i++;


		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid listdocout");
							myInputs[j].focus();
							return false;
						}
		...
		<fieldset>
          ...
          <fieldset>
            <legend>Task Outputs</legend>
                            		<label for="name">listdocout</label>
                            		<div class="div_texbox">
                              		<input name="listdocout" type="text" class="textbox" id="listdocout" value="${listdocout}" />
                            		</div>
